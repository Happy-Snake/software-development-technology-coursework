...
void PieChart::dataChanged(const QModelIndex &topLeft,
...
bool PieChart::edit(const QModelIndex &index, EditTrigger trigger, QEvent *event)
{
    if (index.column() == 0)
...
QRect PieChart::itemRect(const QModelIndex &index) const
{
    if (!index.isValid())
        return QRect();
...
QRegion PieChart::itemRegion(const QModelIndex &index) const
{
    if (!index.isValid())
        return QRegion();
...
int PieChart::rows(const QModelIndex &index) const
{
...
void PieChart::scrollContentsBy(int dx, int dy)
{
    viewport()->scroll(dx, dy);
}

void PieChart::scrollTo(const QModelIndex &index, ScrollHint)
{
...
void PieChart::updateGeometries()
{
